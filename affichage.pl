...
afficheLigne([],_,_):-!.
afficheLigne(_,6,1):-findTypeCase(6,1,TYPE), affichePiece(TYPE, 6, 1),!.
afficheLigne([_|Q],6,Y):-findTypeCase(6,Y,TYPE), affichePiece(TYPE,6,Y),nl,Y1 is Y-1, X1 is 1, afficheLigne(Q,X1,Y1).
afficheLigne([_|Q],X,Y):-findTypeCase(X,Y,TYPE), affichePiece(TYPE,X,Y),tab(1), X1 is X+1, afficheLigne(Q,X1,Y).

%Affiche tout le plateau
affichePlateau:-plateau(P), write(6),tab(1), afficheLigne(P,1,6), nl, write(5),tab(1), afficheLigne(P,1,5), nl, write(4),tab(1), afficheLigne(P,1,4), nl, write(3),tab(1), afficheLigne(P,1,3), nl, write(2),tab(1), afficheLigne(P,1,2), nl, write(1),tab(1), afficheLigne(P,1,1), nl, tab(2), write(" a "), tab(1), write(" b "), tab(1), write(" c "), tab(1), write(" d "), tab(1), write(" e "), tab(1), write(" f "),nl.
