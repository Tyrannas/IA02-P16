...
initTest:- assert(jRouge(humain)), assert(jOcre(humain)), setPlateauGauche,assert(joueurCourant(rouge)), resetPions, placementOrdiRouge,placementOrdiOcre.
...
nbPionsOcre(R):-findall(X, sbireO(X,_), L), length(L,N), R is 6-N.
...
findCaseDeType([[X|[Y|_]]|Q],Type,Liste,Res):-parse(Case,X,Y),findTypeCase(X,Y,Type2),checkType(Type,Type2,Case,OutPut),append(Liste,OutPut,Liste2),findCaseDeType(Q,Type,Liste2,Res).
...
test([],Res,Res).
test([[A,B]|Q], Acc,Res):- write(A), write(B), nl,test(Q,Acc,Res).